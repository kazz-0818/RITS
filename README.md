...
このリポジトリは **Veliora** の AI 人事部エージェント（**RITS**）です。組織共通の規約・registry・手順は [`AGENTS.md`](AGENTS.md) と [`docs/veriora-architecture.md`](docs/veriora-architecture.md) を参照してください。環境変数テンプレートは [`.env.example`](.env.example) です。
